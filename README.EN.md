...
Current version: **v1.4.0** · Node backend + Vue 3 web + Android app · Android scrcpy 4.0 WebSocket + HarmonyOS HDC JPEG + iOS WDA MJPEG cast
...
- **Multi-app cast**: new cast mode hides the left settings pane and fills the workspace with a Windows-style desktop; Win11 taskbar (Start, combined Wi-Fi/volume Quick Settings, clock with lunar calendar); notification center polls device notifications every second while open; Quick Settings controls Wi-Fi / Bluetooth / Airplane mode and volume / brightness from the device
