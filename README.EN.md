...
Current version: **v1.10.0** · Node backend + Vue 3 web + Android app · Android scrcpy 4.0 WebSocket + HarmonyOS HDC JPEG + iOS WDA MJPEG cast
...
- **Multi-app cast**: new cast mode hides the left settings pane and fills the workspace with a Windows-style desktop; Win11 taskbar and Start menu (search + app grid from device launchers); launching an app opens a window with **its own virtual display and start_app** (drag / resize / minimize / maximize); windows auto-close when the app's visible activity ends; if virtual display creation fails (e.g. Android 15 OEM missing ADD_TRUSTED_DISPLAY), a dialog offers close / retry / switch to mirror cast; Icon Helper progress modal only on first setup, then silent background sync for cached icons; Quick Settings controls Wi-Fi / Bluetooth / Airplane mode and volume / brightness; notification center polls device notifications every second while open
