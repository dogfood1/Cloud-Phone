...
Current version: **v1.18.0** · Node backend + Vue 3 web + Android app · Android scrcpy 4.0 WebSocket + HarmonyOS HDC JPEG + iOS WDA MJPEG cast
...
- **Multi-app cast**: Windows-style desktop; **one scrcpy-server per device**, per-window WebSocket / virtual display + `start_app` (aligned with official `--new-display`); encode **60fps / 8Mbps**; server CSD + client SPS/PPS inlining for reliable WebCodecs paint; closing a window runs `am force-stop` on the package; Start menu footer **Fullscreen / Exit fullscreen** (native Fullscreen API, same as mirror cast); on device online, prefetch package/icons into browser cache; Start menu and App Manager are cache-first; Icon Helper / Quick Settings / notification center
