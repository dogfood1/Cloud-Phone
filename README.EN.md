...
Current version: **v1.15.1** · Node backend + Vue 3 web + Android app · Android scrcpy 4.0 WebSocket + HarmonyOS HDC JPEG + iOS WDA MJPEG cast
...
- **Multi-app cast**: Windows-style desktop; **one scrcpy-server per device**, per-window WebSocket / virtual display; encode aligned with mirror (**60fps / 5Mbps**); Start menu opens windows immediately; resize/stream drops reconnect in-window (no false close); Annex-B multi-NAL split; backlog never drops keyframes; soft session restore after restart; browser-cached Start menu icons; Icon Helper / Quick Settings / notification center
