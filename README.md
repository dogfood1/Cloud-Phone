...
当前版本：**v1.10.0** · Node 后端 + Vue 3 Web + Android 客户端 · Android scrcpy 4.0 WebSocket + 鸿蒙 HDC JPEG + iOS WDA MJPEG 投屏
...
| **多应用投屏** | 投屏模式下拉选「多应用投屏」：左侧栏隐藏、右侧全宽 Windows 桌面；Win11 风格任务栏与开始菜单（搜索/应用网格）；从开始菜单打开应用即新建窗口，**每个窗口独立虚拟屏并在该屏启动对应 App**（可拖动/缩放/最小化/最大化）；应用退出后自动关窗；虚拟屏创建失败时弹窗说明并可切换镜像投屏；Icon Helper 仅首次弹窗，之后静默同步缓存图标；快速设置可控制 Wi‑Fi/蓝牙/飞行模式与音量/亮度；通知中心打开时每秒同步设备通知 |
