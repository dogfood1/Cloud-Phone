...
当前版本：**v1.20.4** · Node 后端 + Vue 3 Web + Android 客户端 · Android scrcpy 4.0 WebSocket + 鸿蒙 HDC JPEG + iOS WDA MJPEG 投屏
...
- **多应用模式**：工作区嵌入 Win11 风格桌面（任务栏居中、开始菜单、可拖拽/缩放窗口、每窗独立虚拟屏）；拖动与边角缩放手势稳定；开始菜单可进入独立全屏桌面 Activity，再「取消全屏」返回；快速设置 / 时钟通知从任务栏向上弹出，日期时间与月历对齐 Web，弹出层随屏幕自适应
...
- **ADB 终端**：WebSocket 连接 `adb shell -tt`，ANSI 彩色输出，支持重连
