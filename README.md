...
当前版本：**v1.7.0** · Node 后端 + Vue 3 Web + Android 客户端 · Android scrcpy 4.0 WebSocket + 鸿蒙 HDC JPEG + iOS WDA MJPEG 投屏
...
| **多应用投屏** | 投屏模式下拉选「多应用投屏」：左侧栏隐藏、右侧全宽 Windows 桌面；Win11 风格任务栏与开始菜单（搜索/应用网格）；快速设置可控制 Wi‑Fi/蓝牙/飞行模式与音量/亮度；通知中心打开时每秒同步设备通知；应用图标/名称经授权安装的 Icon Helper 提取（可拒绝，设置可改） |
