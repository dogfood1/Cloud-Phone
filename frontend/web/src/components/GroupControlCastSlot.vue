...
const emit = defineEmits(["toggle-select", "control-relay"]);
...
let getScreenSizeFn = () => ({ width: 0, height: 0 });
...
const {
  ...
  onControlSent: (buffer) => {
    ...
    if (props.batchMode && props.batchRole === "master") {
      ...
      emit("control-relay", {
        ...
        sourceSerial: props.device.serial,
...
getScreenSizeFn = getEffectiveScreenSize;
...
const slotApi = {
  ...
  isCastReady: () => props.selected && status.value === "streaming",
...
const isRelayTarget = computed(() => props.batchMode && props.batchRole === "follower");



function registerWithParent() {

  props.registerSlot?.(props.device.serial, slotApi);
...
onBeforeUnmount(() => {
  ...
  props.registerSlot?.(props.device.serial, null);
...
<template>
  ...
  <article
    ...
    :class="{
      ...
      'group-control-slot--selected': selected,

      'group-control-slot--batch-master': batchMode && batchRole === 'master',

      'group-control-slot--batch-follower': batchMode && batchRole === 'follower',
  ...
  >
    ...
    <div class="group-control-slot__card">
      ...
      >
        ...
        <span v-if="batchMode && batchRole === 'master'" class="group-control-slot__badge">

          {{ t("groupControl.batch.masterBadge") }}
      ...
      <div
        ...
        :class="{
          ...
          'group-control-cast__screen--live': selected,

          'group-control-cast__screen--relay': isRelayTarget,
